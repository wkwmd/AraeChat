...
## 🔒 ZK Extensions (EAMEN V3 / Guillotine)

### X-ZK-01 — ZK Statement Lock Validation (`spec/zk_statement.lock.md`)
**Goal:** Prevent silent drift of the sealed ZK statement / ABI / bounds.
**Pass criteria:**
- `spec/zk_statement.lock.md` contains required sealed markers:
  - `ZK STATEMENT LOCK`
  - `Hash function (sealed): SHA-256`
  - `Max input size (sealed): 1,048,576 bytes (1 MiB)`
  - `Public I/O ABI P-ABI-1 (sealed)` with `Total length: 113 bytes.`
  - Failure rule explicitly seals `out_bytes := ε` and `H_out == SHA256(ε)`
**CI entrypoint:**
- `ci/tools/gen_from_zk_lock.py` 및 `ci/tools/xzk01_check.sh`
**Evidence (CI artifacts):**
- Linux: `ci/logs/xzk01_check_linux.txt`
- macOS: `ci/logs/xzk01_check_macos.txt`
- Windows: `ci/logs/xzk01_check_windows.txt`
